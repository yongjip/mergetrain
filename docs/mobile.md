# Manage trainyard from your phone

trainyard is a local CLI: the queue, the git worktrees, and the deploy all run on your machine. The cleanest way to drive it from a phone is **Cowork Dispatch** — you message a task from the Claude mobile app, and it runs on your Mac/PC Desktop, where it can run `trainyard` against your repo and message you the result. No server to host, no SSH tunnels.
...
- `trainyard` installed on the machine (`pipx install /path/to/trainyard`) and a repo with `.trainyard.yaml`.
...
## How it works with trainyard

You send a short message from your phone. On your Mac, Claude reads [`CLAUDE.md`](../CLAUDE.md), runs the right `trainyard` commands in your repo, and replies with a short summary (plus a push notification when it's done or needs your go-ahead).
...
|---|---|---|
| Check the queue | "trainyard 큐 상태 알려줘" | "What's in the trainyard queue?" |
...
The agent will run the matching commands (e.g. `scripts/ty-status.sh`, `trainyard run-batch --validate-only`) and, for deploy, will show you the plan and wait for your confirmation first.
...
Dispatch gives your phone a path to real actions on your computer — including running deploys, and reading/moving/deleting files. trainyard's design helps (every deploy needs an explicit `--deploy`, and `CLAUDE.md` requires confirmation first), but treat it seriously:
...
> Note: **Claude Code on the web** runs in Anthropic's cloud, not on your machine, so it can't see your local trainyard queue/SQLite — it's the wrong tool for managing a local deploy train.
