...
| `recover_stranded_claim` | Inspect recovery state; require confirmation before applying it. |
...
| `enqueue_clean_branch` | Enqueue only a committed, clean task branch with SHA capture. |
...
| `no_validated_train` | No deploy-eligible validated train is pending. |
| `train_not_found` | The requested train is absent or no longer deploy-eligible. |
