"""Configuration loading for trainyard.

trainyard deliberately keeps runtime dependencies at zero. If PyYAML is
installed it is used; otherwise a small YAML subset parser handles the project
configuration shape generated by ``trainyard init``.
"""
...
DEFAULT_CONFIG_NAME = ".trainyard.yaml"
...
def load_config(
    *,
    ...
    state = StateConfig(
        db=_resolve_path(repo_path, db_value, ".trainyard/queue.sqlite"),
        logs=_resolve_path(repo_path, state_data.get("logs"), ".trainyard/logs"),
        worktree_root=_resolve_path(repo_path, state_data.get("worktree_root"), ".trainyard/worktrees"),
    )
